<%@ WebHandler Language="C#" Class="AddQuestion" %>
...
//处理新增试题的问题
public class AddQuestion : IHttpHandler, IReadOnlySessionState
{
    public void ProcessRequest(HttpContext context)
    {
        ...
        {
            DataFormatSuccess = false;
            result.AppendLine("上传的数据中不包含试题题目");
        }
        ...
        {
            DataFormatSuccess = false;
            result.AppendLine("上传的试题中不包含选择A");
        }
        ...
        {
            DataFormatSuccess = false;
            result.AppendLine("上传的试题中不包含选择B");
        }
        ...
        {
            DataFormatSuccess = false;
            result.AppendLine("上传的试题中不包含选择C");
        }
        ...
        {
            DataFormatSuccess = false;
            result.AppendLine("上传的试题中不包含选择D");
        }
        ...
        {
            DataFormatSuccess = false;
            result.AppendLine("上传的试题中不包含参考答案");
        }
        ...
        {
            DataFormatSuccess = false;
            result.AppendLine("上传的试题中不包含难度系数数据");
        }
        ...
        {
            DataFormatSuccess = false;
            result.AppendLine("上传的试题中不包含试题类型数据");
        }
        ...
        {
            DataFormatSuccess = false;
            result.AppendLine("上传的试题中不包含参考教材数据");
        }
        ...
        if (TextBookId > 0 && TextBookId != null)
        {
            ...
            {
                DataFormatSuccess = false;
                result.AppendLine("上传的试题中不包含参考章节数据");
            }
        }
        ...
        //处理UserId,直接从session中取
        if (context.Session["User"] != null)
        {
            question.UserId = ((Users)context.Session["User"]).UserId;
        }
        else
        {
            DataFormatSuccess = false;
            result.AppendLine("用户没有登录");
        }
        ...
        if (DataFormatSuccess)
        {
            try
            {
                question.QuestionId = new QuestionManager().Add(question);
                result.AppendLine(question.QuestionId.ToString());
            }
            catch (Exception ex)
            {
                result.AppendLine("数据格式正确，但新增试题错误。");
            }
        }
